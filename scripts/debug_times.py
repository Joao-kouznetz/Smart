...
for link in graph.get("links", []):
    ...
    try:
        ...
        errors.append((real_time, inferred_time, error_perc))
    except:
        pass

for r, i, e in errors[:20]:
    print(f"Real: {r:.2f}s | Inferred: {i:.2f}s | Error: {e*100:.2f}%")
print(f"Mean Error: {np.mean([e for r, i, e in errors])*100:.2f}%")
