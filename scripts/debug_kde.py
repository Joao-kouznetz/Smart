...
# Sort by count descending to see KDE
errors.sort(key=lambda x: x[3], reverse=True)

print("--- Top Volumes (KDE >= 50) ---")
for r, i, e, c in errors[:15]:
    print(f"Real: {r:.2f}s | Inferred: {i:.2f}s | Error: {e*100:.2f}% | Count: {c}")

print("\n--- Mid Volumes (IQR 16-49) ---")
mid_errors = [x for x in errors if 16 <= x[3] <= 49]
for r, i, e, c in mid_errors[:15]:
    print(f"Real: {r:.2f}s | Inferred: {i:.2f}s | Error: {e*100:.2f}% | Count: {c}")
...
print(f"Mean Error IQR: {np.mean([e for r, i, e, c in mid_errors])*100:.2f}%")
